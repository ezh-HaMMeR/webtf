...
- **ezquake-tf / ezQuake** — GNU General Public License v2 or later. The default build pins commit
...
- **vcpkg-built libraries** — each package installs its own copyright information into the local
  build tree. The current build uses Expat, libjpeg-turbo, Jansson, MiniZip, PCRE2, libpng,
...
repository. A local build stages it from the compatible ezQuake client path supplied by the user.
